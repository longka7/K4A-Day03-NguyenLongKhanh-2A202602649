...
if __name__ == "__main__":
    ...
    sched_tool = next((t for t in tools if t.get("name") == "submit_leave_request"), None)
    if sched_tool and not sched_tool.get("parameters", {}).get("properties"):
        print("⏳ [TODO 1.2]: Tool 'submit_leave_request' chưa được định nghĩa properties trong 'src/tools.py'.")
    else:
        print("✅ [TODO 1.2]: Tool 'submit_leave_request' đã có schema đầy đủ.")

    # Kiểm tra trạng thái TODO 2.1 (call_tool)
    test_result = server.call_tool("hr_query", {"employee_id": "NV2026001"})
    if not test_result:
